    ...
    def learn_top_k(self, original_tree: PatternTree, target_tree: PatternTree, k: int):
                ...
                for target_node in transformed_nodes:
                    ...
                    token_mapping_results.append(
                        [
                            ...
                            for result in zip(
                                validated_original_to_transformed_tuples_by_pattern,
                                scores_by_pattern,
                            )
                        ]
                    )
        ...
        for original_node in original_nodes[num_possible_nodes:]:
            for i in range(k):
                transform_result = TransformedResult(
                    [(str_value, "") for str_value in original_node.value.values], 0.0
                )
                final_node_to_results[original_node] = [transform_result]

        validated_original_to_transformed_tuples_by_pattern = [
            [] for _ in range(len(final_node_to_results))
        ]
        ...
        for node, results in final_node_to_results.items():
            self.validator.validate_results(
                results[0].original_target_pairs, target_tree
            )
            ...
            for idx1, result in enumerate(results):
                for idx2, (original_value, transformed_value) in enumerate(
                    ...
                    if idx1 == 0:
                        ...
                        current_values = validated_original_to_transformed_tuples_by_pattern[
                            idx
                        ][
                            idx2
                        ][
                        ...
                        validated_original_to_transformed_tuples_by_pattern[idx][idx2][
                            1
