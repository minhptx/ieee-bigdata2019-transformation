...
from datafc.transform.pattern_mapping import PatternMappingModel
...
class TransformationModel:
    def __init__(self, mapping_method="sim", string_similarity="cosine"):
        self.pattern_mapper = PatternMappingModel(mapping_method, string_similarity)
        ...
        for original_to_transformed_pairs in original_to_transformed_pairs_list:
            for idx, (original_value, transformed_value, valid_result) in enumerate(original_to_transformed_pairs):
                if len(original_to_transformed_results) <= idx:
                    ...
                    assert (
                        original_to_transformed_results[idx][0] == original_value
