...
class PatternMappingModel:
    def __init__(self, method="sim", string_similarity="cosine"):
        self.mapping_method = method

        if method == "sim":
            self.token_mapper: TokenSimMappingModel = TokenSimMappingModel(string_similarity)
        elif method == "attr":
    ...
    def learn_top_k(self, original_tree: PatternTree, target_tree: PatternTree, k: int):
                ...
                for target_node in transformed_nodes:
                    ...
                    token_mapping_results.append(
                        [
                            ...
                            for result in zip(validated_original_to_transformed_tuples_list, scores_list)
                        ]
                    )

                node_to_results[original_node] = max(token_mapping_results, key=lambda x: x[0].score)

                pattern_scores.append(np.mean([x.score for x in node_to_results[original_node]]))
        ...
        return validated_original_to_transformed_tuples_list, [sum(scores) for scores in scores_list]
