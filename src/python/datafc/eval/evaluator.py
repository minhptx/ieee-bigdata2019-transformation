...
class Evaluator:
    def __init__(
        ...
        self.name_to_active_result: Dict[
            str, List[TopKTransformationResult]
        ] = defaultdict(lambda: [TopKTransformationResult() for _ in range(30)])
    ...
    def micro_accuracy(self, name):
        return (
            self.name_to_result[name].correct_count
            * 1.0
    ...
    def mrr_score(self, name):
        return (
            self.name_to_result[name].mrr_count / self.name_to_result[name].total_count
        )

    def exact_accuracy(self):
        return sum(
            [x.is_correct_transformation for x in self.name_to_result.values()]
        ) / len(self.name_to_result)
    ...
    def macro_mean_active_accuracy(self, step):
        return sum(
            [self.micro_active_accuracy(name, step) for name in self.name_to_result]
        ) / len(self.name_to_result)
    ...
    def macro_mean_top_k_accuracy(self):
        return sum(
            [self.micro_top_k_accuracy(name) for name in self.name_to_result]
        ) / len(self.name_to_result)
    ...
    def check_transformation_result(
        ...
        for (
            ...
            transformed_value,
            validated_result,
        ) in original_to_transformed_values:
    ...
    def run_normal_experiment(
        ...
        self.check_transformation_result(
            ...
            validated_original_to_transformed_values,
        )
